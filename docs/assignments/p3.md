...
- **State:** 
    * tags: set Tag  
    * name: tags \-\> one string  
    * taggedItems: tags \-\> set Item  

- **Actions:**
    * *createTag*(name: string)  
      * tag is created and added to the list of tags  
    * *tagItem*(item: Item, tag: Tag)  
      * item is added to the in tags   
    * *getItemsWithTag*(tag: Tag)  
      * finds tag in tags, then returns taggedItems for that tag  
...
**Nice to have:** Bookmarking, Mapping

### Concept Diagram
![](./project-images/concept_diagram.png)
...
sync listItem(author: user, name: string, qt: integer,  location: string, img: file, tags: set Tag, expirationDate: Date){
	listing = Listing.add(author, name, qt, location, img)
	Tagging.tagItem(item, tag)
